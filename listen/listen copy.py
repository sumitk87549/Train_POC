...
def check_dependencies():
    ...
    optional = {
        ...
        'chatterbox': 'chatterbox-tts',  # Chatterbox TTS (optional)
        'parler_tts': 'parler-tts (install from: pip install git+https://github.com/huggingface/parler-tts.git)',  # ParlerTTS (optional)
    }
    ...
    "INTERMEDIATE": {
        "huggingface": [
            ...
            },
            {
                ...
                "lang": "multilingual",
                "type": "chatterbox",
                "description": "Chatterbox - state-of-art multilingual TTS with emotion control (23+ languages)",
                "ram": "4-8GB",
                "quality": "Excellent, natural human-like speech with configurable emotion"
            },
            {
                "name": "ai4bharat/indic-parler-tts",
                "lang": "multilingual",
                "type": "parler",
                "description": "Indic ParlerTTS - controllable TTS for 20 Indic languages + English",
                "ram": "4-8GB",
                "quality": "High quality with voice characteristic control"
            }
        ],
        "coqui": [
            {
...
class TTSEngine:
    ...
    def __init__(self, model_name, model_type, device="cpu", tier="BASIC"):
        ...
        self.processor = None
        self.tokenizer = None
        self.description_tokenizer = None  # For ParlerTTS
        self.chatterbox_model = None  # For Chatterbox
    ...
    def load_model(self):
        ...
        try:
            if self.model_type == "vits":
                ...
                return self._load_parler()
            else:
                raise ValueError(f"Unknown model type: {self.model_type}")
        except ImportError as e:
            if "chatterbox" in str(e).lower():
                print("❌ Chatterbox TTS not installed. Install with: pip install chatterbox-tts")
            elif "parler" in str(e).lower():
                print("❌ Parler TTS not installed. Install with: pip install git+https://github.com/huggingface/parler-tts.git")
            raise
    ...
    def _load_chatterbox(self):
        """Load ResembleAI Chatterbox model."""
        if not CHATTERBOX_AVAILABLE:
            raise ImportError("Chatterbox TTS not installed. Install with: pip install chatterbox-tts")
    ...
    def _load_parler(self):
        """Load ParlerTTS model with dual tokenizer system."""
        if not PARLER_AVAILABLE:
            raise ImportError("ParlerTTS not installed. Install with: pip install git+https://github.com/huggingface/parler-tts.git")
        ...
        # ParlerTTS requires two tokenizers
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        self.description_tokenizer = AutoTokenizer.from_pretrained(
            self.model.config.text_encoder._name_or_path
        )
        
        print("✅ ParlerTTS model loaded (dual tokenizer)")
        return True
    ...
    def generate_audio(self, text, output_path):
        ...
        if self.model_type == "vits":
            ...
            return self._generate_chatterbox(text, output_path)
        elif self.model_type == "parler":
            return self._generate_parler(text, output_path)
    ...
    def _generate_chatterbox(self, text, output_path):
        """Generate audio using ResembleAI Chatterbox."""
        print(f"🎙️  Generating audio with Chatterbox...")

        # Generate audio
        wav = self.chatterbox_model.generate(text)
        
        # Save using torchaudio
        ta.save(output_path, wav, self.chatterbox_model.sr)
        
        print(f"✅ Audio generated: {output_path}")
        return output_path


    def _generate_parler(self, text, output_path):
        """Generate audio using ParlerTTS with voice description."""
        print(f"🎙️  Generating audio with ParlerTTS...")

        # Define voice description for Hindi/Indic languages
        # This controls the voice characteristics
        description = "A clear Hindi speaker with natural tone delivers speech with moderate speed and pitch. The recording is of high quality with clear pronunciation."
        ...
        # Save audio (ParlerTTS uses model config for sample rate)
        sample_rate = self.model.config.sampling_rate
        ...
        if self.args.model:
            ...
            if not model_info:
                ...
                if 'mms-tts' in model_name:
                    ...
                    model_type = "speecht5"
                elif 'chatterbox' in model_name.lower():
                    model_type = "chatterbox"
                elif 'parler' in model_name.lower() or 'indic-parler-tts' in model_name:
                    model_type = "parler"
                else:
